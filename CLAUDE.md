...
## 실행
- `/design-orchestrator` → Phase 1~9 자동 실행 (핵심 커맨드)
- 내부에서 필요 시 호출하는 스킬: `requirements-preprocessor`, `requirements-analyzer`, `doc-sync`, `pdf`
...
- 업무규칙: `BR-{도메인}-{NNN}` (예: BR-USER-001)
- TC: `TC-{기능ID}-{NNN}` (예: TC-USER-001-001)
- NFR: `NFR-{카테고리}-{NNN}` (예: NFR-PERF-001)
- 에러: `ERR-{도메인}-{NNN}`
- 인터페이스: `IF-{대상}-{NNN}` (예: IF-SAP-001)
