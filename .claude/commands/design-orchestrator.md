...
### Step 1.1: 입력물 수집 및 정리
- docs/00_srs/ 의 모든 파일 읽기
- **PDF 입력 처리 (🔀 병렬 검증):**
  1. 사용자가 PDF 전체를 복붙한 .md 파일을 읽고 섹션별로 정리 (표 복원, 구조화)
  2. 서브에이전트가 원본 PDF를 /pdf 스킬로 동시에 읽기
  3. 복붙 정리본 ↔ PDF 읽기 결과를 교차 검증 (누락/불일치 탐지)
  4. 차이 발견 시 사용자에게 확인
  5. 검증된 정리본으로 이후 Phase 진행
