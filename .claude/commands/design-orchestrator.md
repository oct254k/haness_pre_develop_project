...
### Step 1.1: 입력물 확인
- docs/00_srs/rfp-final.md 존재 확인
- **없으면**: "`/prepare-rfp` 먼저 실행해주세요" 안내 후 대기
- **있으면**: rfp-final.md + docs/00_srs/ 내 추가 파일 읽기
...
### Step 6.3: 디자인 시스템 추출
- **사용자에게 디자인 템플릿 요청:**
  - Figma 링크 / HTML 템플릿 / CSS 파일 / 스크린샷 등
  - "사용할 디자인 템플릿이 있나요? (Figma, HTML, 기존 사이트 URL 등)"
  - 없으면: 기술 스택 기반 기본 디자인 시스템 생성 (반응형 필수 포함)
- **템플릿에서 추출할 내용:**
  - 색상 팔레트 (primary, secondary, gray scale, semantic)
  - 타이포그래피 (폰트, 크기 체계, line-height)
  - 간격 체계 (spacing scale)
  - 공통 컴포넌트 패턴 (버튼, 입력, 테이블, 모달, 카드 등)
  - 레이아웃 패턴 (그리드, 사이드바, 헤더 구조)
  - 반응형 브레이크포인트 (모바일/태블릿/��스크톱 필수)
...
- 각 SCR-NNN.md에 포함할 내용:
  ...
  **2. 와이어프레임**
  ...
  - 반응형 레이아웃 변화 (데스크톱 → 태블릿 → 모바일에서 어떻게 바뀌는지)
...
### Step 9.6: 핵심 데이터 요약서 생성
- 모든 문서가 검증된 후, **개발자가 바로 참조할 핵심 데이터만** 추출
- docs/_meta/dev-summary.md 생성:
  ```markdown
  # 개발 핵심 요약
  > 자동 생성: {일시} | 전체 문서 검증 완료 후 추출

  ## 도메인 목록
  | 도메인 | 기능 수 | 화면 수 | API 수 | 핵심 엔티티 |

  ## 기능 → 화면 → API 매핑 (전체)
  | F-ID | 기능명 | SCR | API | TC 수 |
  ...
  ## 에러코드 전체 목록
  | ERR-ID | HTTP | 메시지 | 발생 조건 |
  ```
- 이 문서는 **다른 문서를 읽지 않고도 전체 프로젝트를 파악할 수 있는 한 장짜리 요약**
